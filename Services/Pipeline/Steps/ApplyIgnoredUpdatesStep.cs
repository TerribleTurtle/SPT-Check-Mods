...
public sealed class ApplyIgnoredUpdatesStep(
    ...
    ILogger<ApplyIgnoredUpdatesStep> logger) : IWorkflowStep
{
    /// <inheritdoc />
    public async Task ExecuteAsync(UpdateWorkflowContext context, CancellationToken cancellationToken)
    {
        ...
        var modsWithIgnores = await updateOrchestrationService.ApplyIgnoredUpdatesAsync(context.Mods, cancellationToken);
