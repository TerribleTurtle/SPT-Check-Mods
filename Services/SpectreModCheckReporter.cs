...
public sealed class SpectreModCheckReporter : IModCheckReporter
{
    ...
    /// <inheritdoc />
    public Task RunForgeQueryProgressAsync(int total, Func<Action<int>, Task> work)
    {
        return _progressRenderer.RunForgeQueryProgressAsync(total, work);
    }

    /// <inheritdoc />
    public Task<T> RunForgeQueryProgressAsync<T>(int total, Func<Action<int>, Task<T>> work)
    {
        return _progressRenderer.RunForgeQueryProgressAsync(total, work);
    }
    ...
    /// <inheritdoc />
    public void LoadingWarnings(List<Mod> modsWithWarnings) { _tableRenderer.LoadingWarnings(modsWithWarnings); }
    
    /// <inheritdoc />
    public void ReconciliationResults(ModReconciliationResult result) { _tableRenderer.ReconciliationResults(result); }
    
    /// <inheritdoc />
    public void MisplacedMods(MisplacedModReport report) { _tableRenderer.MisplacedMods(report); }
    
    /// <inheritdoc />
    public void UnverifiedMods(List<Mod> mods) { _tableRenderer.UnverifiedMods(mods); }
    
    /// <inheritdoc />
    public void DependencyResults(DependencyAnalysisResult result) { _tableRenderer.DependencyResults(result); }
    
    /// <inheritdoc />
    public void VersionTable(List<Mod> mods) { _tableRenderer.VersionTable(mods); }

    /// <inheritdoc />
    public bool PromptFetchRemoteIgnores() { return _textRenderer.PromptFetchRemoteIgnores(); }
    ...
    /// <inheritdoc />
    public bool PromptReportIgnores() { return _textRenderer.PromptReportIgnores(); }
